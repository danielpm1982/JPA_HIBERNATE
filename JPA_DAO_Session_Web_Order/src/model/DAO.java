...
//				session.delete(orderToDelete); //optional, deletes the null-valued order at the table. Cascading delete does not apply here 'cause what has been deleted at the Customer class has been a list item (which is not cascaded), not a Customer instance. No need if orphanRemoval=true is used at the Customer entity @oneToMany relationship attribute ('order'), in which case the removal of each order, with null fk, from the table (after being removed from the Customer orderList) would be done automatically, which is preferred.
	...
	public static void addItemToOrder(long customerId, String orderName, Item item, SessionFactory factory) {
		Session session = factory.getCurrentSession(); //must get a new session after each commit(), which closes the transaction and the session (differently from when using EntityManager and EntityManagerFactory, when only the transaction is closed).
		session.getTransaction().begin();
		checkResources(session, factory);
		Customer customer = session.get(Customer.class, customerId);
		if(customer!=null) {
			System.out.println("Inserting item: "+item+" to order: "+orderName+" of customer: "+customer+"...");
			Order order = customer.getOrder().stream().filter(x->x.getName().equals(orderName)).findFirst().orElse(null); 
			if(order!=null) {
				order.addItem(item);
				System.out.println("Item sucessfully inserted!");
			} else {
				System.out.println("Order not found for that orderName!");
			}
			session.getTransaction().commit();
		} else {
			session.close();
	...
	public static void deleteItemFromOrder(long customerId, String orderName, String itemName, SessionFactory factory) {
		Session session = factory.getCurrentSession(); //must get a new session after each commit(), which closes the transaction and the session (differently from when using EntityManager and EntityManagerFactory, when only the transaction is closed).
		session.getTransaction().begin();
		checkResources(session, factory);
		Customer customer = session.get(Customer.class, customerId);
		if(customer!=null) {
			System.out.println("Deleting item(s): "+itemName+" from order: "+orderName+" of customer: "+customer+"...");
			Order order = customer.getOrder().stream().filter(x->x.getName().equals(orderName)).findFirst().orElse(null); 
			if(order!=null) {
				List<Item> items = order.getItemList().stream().filter(x->x.getName().equals(itemName)).collect(Collectors.toList());
				if(items!=null&&!items.isEmpty()) {
					items.forEach(x->order.removeItem(x));
					if(!order.getItemList().stream().filter(x->x.getName().equals(itemName)).findFirst().isPresent()) {
						System.out.println("Item(s) sucessfully deleted!");
					}
				}
			} else {
				System.out.println("Order not found for that orderName!");
	...
	public static Customer findCustomerByIdWithAllItems(long customerId, SessionFactory factory) { //returns a specific Customer Id object with all order items fetched from the Item table. While fetching, a Session and Transaction are needed, later, for just using the object, it is not.
		System.out.println("Getting new Session and activating Transaction...");
		Session session = factory.getCurrentSession(); //must get a new session after each commit(), which closes the transaction and the session (differently from when using EntityManager and EntityManagerFactory, when only the transaction is closed).
		checkResources(session, factory);
		System.out.println("Retrieving entity at id="+customerId+"...");
		...
		if(customer!=null) {
			...
			if(!customer.getOrder().isEmpty()) {
				...
				customer.getOrder().forEach(x->System.out.println(!x.getItemList().isEmpty()?(x+" items: "+x.getItemList()+" total price: $"+x.getTotalPrice()):"No Items for that Order!"));
				session.getTransaction().commit();
			} else {
...
//		}
	...
	public static boolean truncateTable(SessionFactory factory) {
		...
		try {
			...
			session = factory.getCurrentSession();
			session.getTransaction().begin();
			System.out.println("Truncating table 'scheme1.item':");
			session.createNativeQuery("truncate scheme1.item").executeUpdate();
			System.out.println("Successfully truncated!");
			session.getTransaction().commit();
			return true;
		} catch (Exception e) {
