...
//    @Temporal(TemporalType.TIMESTAMP) Does not work on the new java.time api, only with the old date/time/Calendar api.
    ...
    @OneToMany
    @JoinTable(name = "Client_Department",joinColumns=@JoinColumn(name="CLIENT_ID"),inverseJoinColumns=@JoinColumn(name="DEPARTMENT_NO",nullable=true))
    private List<Department> department;
    @Transient
    private String unmappedFakeAttribute; //just for testing transient annotation
    public Client(String name, List<String> phoneNo, BigDecimal salary, LocalDate birthDate, LocalDateTime registeredIn, List<Department> department) {
